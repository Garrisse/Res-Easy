...
<!-- In this file we handle with SQL queries the database logic about the Renting table, the middle table between Resident and Rooms (a resident rents a room).-->
...
<?php
    class Renting_Model {
        ...
        public function getAllByResident($residentId) {
            ...
            $result = $this->connection->query(
                "SELECT rentingTable.*, roomsTable.name as room_name, roomsTable.type as room_type
                 FROM reseasy_renting rentingTable
                 JOIN reseasy_rooms roomsTable ON rentingTable.room_id = rm.room_id
        ...
        public function getActiveByResident($residentId) {
            $residentId = intval($residentId);
            $today = date('Y-m-d');
